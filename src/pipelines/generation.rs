...
use crate::Config;
use crate::pipelines::generation::private_generation_utils::{PrivateLanguageGenerator, MutablePrivateLanguageGenerator};
...
impl MutablePrivateLanguageGenerator<BartForConditionalGeneration, RobertaVocab, RobertaTokenizer> for BartGenerator {
...
impl MutableLanguageGenerator<BartForConditionalGeneration, RobertaVocab, RobertaTokenizer> for BartGenerator {}
...
impl MutablePrivateLanguageGenerator<MarianForConditionalGeneration, MarianVocab, MarianTokenizer> for MarianGenerator {
...
impl MutableLanguageGenerator<MarianForConditionalGeneration, MarianVocab, MarianTokenizer> for MarianGenerator {}
...
mod private_generation_utils {
    ...
    use std::cmp::{min, max};
    use crate::pipelines::generation::{BeamHypotheses, GenerateConfig, LMHeadModel, MutableLMHeadModel};
    use itertools::Itertools;
...
//            Get banned tokens and set their probability to 0
        ...
        fn reorder_cache(&self, past: Option<Vec<Tensor>>, _encoder_outputs: Option<Tensor>, beam_indices: &Tensor) -> (Option<Vec<Tensor>>, Option<Tensor>) {
            match past {
                Some(value) => {
    ...
    pub trait MutablePrivateLanguageGenerator<T: MutableLMHeadModel, V: Vocab, U: Tokenizer<V>> {
...
pub trait MutableLanguageGenerator<T: MutableLMHeadModel, V: Vocab, U: Tokenizer<V>>: MutablePrivateLanguageGenerator<T, V, U> {
    /// Generate text based on a vector of promp texts.
    ///
    /// # Arguments
    ///
    ...
    fn generate(&mut self, prompt_texts: Option<Vec<&str>>, attention_mask: Option<Tensor>)
                -> Vec<String> {
        let eos_token_ids = MutablePrivateLanguageGenerator::get_eos_ids(self).clone();

        let config = MutablePrivateLanguageGenerator::get_config(self);
        let do_sample = config.do_sample;
        let num_return_sequences = config.num_return_sequences;
        let num_beams = config.num_beams;
        let min_length = config.min_length;
